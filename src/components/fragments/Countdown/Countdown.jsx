...
export const CountdownList = ({ data }) => (
    <FlexStart gap="gap-1 lg:gap-3" classname="justify-center">
        {data.map((item, index) => (
            <React.Fragment key={index}>
                <CountdownItem value={item.value} label={item.label} />
                {index < data.length - 1 && (
                    <h6 className="text-base lg:text-2xl text-white font-medium opacity-60">:</h6>
                )}
            </React.Fragment>
        ))}
    </FlexStart>
...
export const CountdownItem = ({ value, label }) => (
    <FlexCenter gap="gap-1" classname="flex-col">
        ...
        <TextCustom
            ...
            texttransform="uppercase"
            textColor="text-white"
            classname="text-xs md:text-[10px] lg:text-xs opacity-80 font-light"
        >
            {label}
        </TextCustom>
    </FlexCenter>
